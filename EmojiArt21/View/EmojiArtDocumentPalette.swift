...
//    @discardableResult
    func removeEmoji(_ emojisToRemove: String, fromPalette palette: String) -> String {
        return changePalette(palette, to: palette.filter {
            !emojisToRemove.contains($0)
        })
    }
    ...
    // in the real world, we'd want palettes to the be identifiable
    // here we're simply guessing that
